...
def main():
    ...
    uav1 = PMAP_UAV(nodes.Get(2), 1)
    uav2 = PMAP_UAV(nodes.Get(3), 2)
    ...
    nodes.Get(2).AddApplication(uav1)
    nodes.Get(3).AddApplication(uav2)

    zsp1.SetStartTime(ns.Seconds(0))
    zsp2.SetStartTime(ns.Seconds(0))
    uav1.SetStartTime(ns.Seconds(0))
    uav2.SetStartTime(ns.Seconds(0))

    # =============================
    # 预注册
    # =============================

    preregister(uav1, [zsp1, zsp2], 1)
    preregister(uav2, [zsp1, zsp2], 2)
    ...
    def start_d2z():
        print("\n===== D2Z PHASE =====\n")
        uav1.D2Z_InitiateAuth()
        uav2.D2Z_InitiateAuth()
    ...
    def start_d2d():
        print("\n===== D2D PHASE =====\n")
        uav1.D2D_InitiateAuth(uav2.pid)

    # =============================
    # 调度（关键时间轴）
    # =============================

    uav1._safe_schedule(2, start_d2z)

    uav1._safe_schedule(7, start_d2d)
